...
import (
	...
	"github.com/specialpowerchain/go-specialiuma/core/state"
	...
	"github.com/specialpowerchain/go-specialiuma/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Specialium protocol")
	...
	log.Info("Light Specialium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Specialium peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Specialium handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		clientErrorMeter.Mark(1)
		p.Log().Error("Light Specialium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Specialium message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Specialium message arrived", "code", msg.Code, "bytes", msg.Size)
