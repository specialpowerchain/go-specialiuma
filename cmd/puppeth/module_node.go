...
{{end}}
RUN \
  echo 'gsp --cache 512 init /genesis.json' > gsp.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.specialium/keystore/ && cp /signer.json /root/.specialium/keystore/' >> gsp.sh && \{{end}}
	echo $'exec gsp --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gsp.sh

ENTRYPOINT ["/bin/sh", "gsp.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.specialium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.sphash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.specialium"],
		ethashdir:  infos.volumes["/root/.sphash"],
		port:       port,
